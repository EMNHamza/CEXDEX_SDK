...
from tools.utilsBalanceTampon import updateBalanceTampon, updateBalancesObject
...
def processOpportunityConsumer(queueOpportunitiesToExecute: Queue, balances: Balances, balanceBuffer:Dict, lock:Lock):
    while True:
        try:
            ...
            if len(listOpportunities)>0:
                newBalanceBuffer = deepcopy(balanceBuffer)

                newBalanceBuffer = updateBalanceTampon(listOpportunities=listOpportunities, balanceBuffer=newBalanceBuffer, isLock=True)

                # logging.info(f'newBalanceBuffer {newBalanceBuffer}')
                with lock:
                    for platform in newBalanceBuffer:
                        balanceBuffer[platform] = newBalanceBuffer[platform]

                # for platform, assets in balanceBuffer.items():
                #     logging.info(f"processOpportunityConsumer - UPDATED balanceBuffer - platform: {platform}")
                #     for asset, value in assets.items():
                #         if float(value) > 0:
                #             logging.info(f" processOpportunityConsumer - UPDATED balanceBuffer - {asset}: {value}")


            for opportunity in listOpportunities:
                # logging.info(f'processOpportunityConsumer - typeOpp : {opportunity}')
                if isinstance(opportunity, OpportunityCexDex):
                    if opportunity.opportunityThorchain.typeOpp=="CEXDEXMAYA":
                        # logging.info(f'opp CEXDEXMAYA')
                        processExecuteOpportunity_ =  Process(target=executeMayaCexDexOpportunity, args=(balances, opportunity, balanceBuffer, lock))

                    elif opportunity.opportunityThorchain.typeOpp=="CEXDEX":
                        processExecuteOpportunity_ = Process(target=executeCexDexOpportunity, args=(balances, opportunity, balanceBuffer, lock))

                    processExecuteOpportunity_.start()
                    

                if isinstance(opportunity, OpportunityThorchain):
                    processExecuteOpportunity_ = Process(target=executeThorchainOnBlockOpportunity,args=(balances, opportunity, balanceBuffer, lock),)
                    processExecuteOpportunity_.start()

                if isinstance(opportunity, OpportunityMaya):
                    processExecuteOpportunity_ =  Process(target=executeMayaOnBlockOpportunity, args=(balances, opportunity, balanceBuffer, lock))
                    processExecuteOpportunity_.start()    
                
                if isinstance(opportunity, OpportunityDexDex):
                    processExecuteOpportunity_ =  Process(target=executeMayaThorOpportunity, args=(balances, opportunity, balanceBuffer, lock))
                    processExecuteOpportunity_.start()                      
    ...
    while True:
        try:
            ...
            if catchingUpNode == True:
                logging.warning("processGetThorchainBlock - catching up true")
                time.sleep(10)
                continue
            ...
            if currentBlock > lastBlock and catchingUpNode == False:
                ...
                if int(newCurrentBlock) > currentBlock:
                    logging.info(
                        f"newCurrentBlock {newCurrentBlock} vs oldBlock {currentBlock}"
                    )
                ...
                # for platform, assets in balanceBuffer.items():
                #     logging.info(f"processGetThorchainBlock - balanceBuffer - platform: {platform}")
                #     for asset, value in assets.items():
                #         if float(value) > 0:
                #             logging.info(f" processGetThorchainBlock - balanceBuffer - {asset}: {value}")


                updateBalancesObject(balances=BalancesShared,balanceBuffer=balanceBuffer)
                ...
                # for symbol, data in orderbookDataShared.items():
                #     logging.info(f"{symbol}: {data}")

                # for asset in BalancesShared.balancesThorchain.listAssets:
                #     if asset.assetType != "RUNE":
                #         logging.info(
                #             f"processGetThorchainBlock - balancesThorchain currentBlock {currentBlock} - asset.symbol : {asset.symbol} - asset.balance {asset.balance} asset.pool.balanceAssetInPool {asset.pool.balanceAssetInPool} asset.pool.balanceRuneInPoolAsset {asset.pool.balanceRuneInPoolAsset}"
                #         )

                # for asset in BalancesShared.balancesThorchain.listAssets:
                #     logging.info(f'processGetThorchainBlock - BalancesShared - balancesThorchain - asset.symbol : {asset.symbol} - asset.balance {asset.balance} asset.decimals {asset.decimals}')
                
                # for asset in BalancesShared.balancesBybit.listAssets:
                #     logging.info(f'processGetThorchainBlock - BalancesShared - balancesBybit - asset.symbol : {asset.symbol} - asset.balance {asset.balance} asset.decimals {asset.decimals}')

                # for asset in BalancesShared.balancesMaya.listAssets:
                #     logging.info(f'processGetThorchainBlock - BalancesShared - balancesMaya - asset.symbol : {asset.symbol} - asset.balance {asset.balance} asset.decimals {asset.decimals}')

                # time.sleep(1.5)
                # poolDataSecond = getThorchainPool()
                # if(poolDataSecond != poolData):
                #     numberOfChanges=numberOfChanges+1
                #     poolData=poolDataSecond
                #     updateAllBalances(BalancesShared)
                #     updateThorchainAssetPoolData(balances=BalancesShared, poolData=poolData, currentBlock=currentBlock)

                #     for asset in BalancesShared.balancesThorchain.listAssets:
                #         if asset.assetType != 'RUNE':
                #             logging.info(f'processGetThorchainBlock - SECOND TRY - balancesThorchain currentBlock {currentBlock} - asset.symbol : {asset.symbol} - asset.balance {asset.balance} asset.pool.balanceAssetInPool {asset.pool.balanceAssetInPool} asset.pool.balanceRuneInPoolAsset {asset.pool.balanceRuneInPoolAsset}')

                # time.sleep(1.5)
                # poolDataThird = getThorchainPool()
                # if(poolDataThird != poolData):
                #     numberOfChanges=numberOfChanges+1
                #     poolData=poolDataThird
                #     updateAllBalances(BalancesShared)
                #     updateThorchainAssetPoolData(balances=BalancesShared, poolData=poolData, currentBlock=currentBlock)

                # for asset in BalancesShared.balancesThorchain.listAssets:
                #     if asset.assetType != 'RUNE':
                #         logging.info(f'processGetThorchainBlock - balancesThorchain currentBlock {currentBlock} - asset.symbol : {asset.symbol} - asset.balance {asset.balance} asset.pool.balanceAssetInPool {asset.pool.balanceAssetInPool} asset.pool.balanceRuneInPoolAsset {asset.pool.balanceRuneInPoolAsset}')

                # if numberOfChanges>1:
                #     logging.info(f'numberOfChanges - {numberOfChanges}')
            ...
            logging.warning(f"processGetThorchainBlock error {err}")
            traceback.print_exc()


def processGetMayaBlock(slidingListBlock, BalancesShared:Balances, orderbookDataShared:Dict, queueOpportunitiesMaya:Queue, queueOpportunitiesToExecute:Queue,balanceBuffer:Dict):
    while True:
        try:
            block = getMayaBlock()
            slidingListBlock.append(int(block))
            ...
            if currentBlock > lastBlock and catchingUpNode == False:
                ...
                currentBlockThor = requests.get(url=URL_BLOCK_THOR, timeout=1).json()["result"][
                "sync_info"
            ]["latest_block_height"]
                # compteur =0
                # while newPoolData == referencePoolData:
...
def launchMayaProcesses(BalancesShared: Balances, orderbookDataShared:Dict, queueOpportunitiesMaya:Queue, queueOpportunitiesToExecute:Queue):
    ...
    processOpportunityMayaVSThorProducer = Process(target=processCreateOpportunityMayaVSThor, args=(BalancesShared,queueOpportunitiesMaya,eventScoutDexDex))
    ...
    try:
        ...
        for pairCexDex in pairsCexDex:
            if pairCexDex.pairAssetDex.assetIn.balance > 0 and pairCexDex.pairAssetCex.assetIn.balance > 0:
                ...
                # logging.info(f'CEXDEX - opp thorchain : amountInInDollars {round(opportunityThorchain.amountInInDollars,2)} $, amountIn {opportunityThorchain.amountIn/1e8} {opportunityThorchain.pairAsset.assetIn.symbol}  amountOut {opportunityThorchain.amountOutEstimated/1e8} {opportunityThorchain.pairAsset.assetOut.symbol}  ')
                ...
                if opportunityThorchain.amountInInDollars > 15:
                    ...
                    # logging.info(f'CREATEBYBITOPP REEL START')
                    ...
                    conditionOpp = (
                        ...
                        > THRESHOLD_GAIN1000_NET
                            ...
                            # logging.info(f'')
                    # logging.info(f'CEXDEX - opportunityCexDex :  gainTotalEstimated {opportunityCexDex.gainTotalEstimated} $, gain1000 {getGain1000CexDex(opportunityCexDex)} $, amount in value in dollars {getAmountInValue(opportunityCexDex)} $')
                    # logging.info(
                    ...
                    # logging.info(f'opportunityCexDex :  gainTotalEstimated {opportunityCexDex.gainTotalEstimated} $, gainAssetInDexEstimated {opportunityCexDex.gainAssetInDexEstimated}, gainAssetOutDexEstimated {opportunityCexDex.gainAssetOutDexEstimated}')
                    # logging.info(f'')

                    if conditionOpp:
                        print("y'a une opp CEXDEX THORCHAIN fréro")
                        logging.info(f"")
                        logging.info(
                        ...
                        # (amountInOpti, gainNetInStable) = dichotomieCexDexLog(
                        #         f=formulaGainStableCexDex,
                        #         low=10,
                        #         high=int(amountInMax*10**pairCexDex.pairAssetDex.assetIn.decimals),
                        #         e=1,
                        #         assetIn=opportunityCexDex.opportunityThorchain.pairAsset.assetIn,
                        #         assetOut=opportunityCexDex.opportunityThorchain.pairAsset.assetOut,
                        #         isSynthSwapIn=True,
                        #         isSynthSwapOut=True,
                        #         orderbookData=pairCexDex.pairAssetCex.orderbook,
        ...
        logging.info(f'SET eventScoutCexDexThorchain')
...
def processCreateOpportunityThorchainOnBlock(
    ...
    try:
        ...
        for pairThorchain in pairsThorchain:
            if pairThorchain.assetIn.assetType == pairThorchain.assetOut.assetType:
                if pairThorchain.assetIn.balance > 0:
                    ...
                    # logging.info(f'processCreateOpportunityThorchainOnBlock -amountInMax {amountInMax} {pairThorchain.assetIn.symbol} to {pairThorchain.assetOut.symbol}')
                    ...
                    opportunityThorchain = createThorchainSynthOnBlockOpportunity(
                        ...
                        # amountIn=int(pairThorchain.assetIn.balance),
                        typeOpp="THORCHAIN",
                        balancesThorchain=BalancesShared.balancesThorchain,
                    )

                    # logging.info(f'ONBLOCK - opp thorchain : assetIn : {opportunityThorchain.pairAsset.assetIn.symbol} assetOut : {opportunityThorchain.pairAsset.assetOut.symbol} amountIn {opportunityThorchain.amountIn/1e8} amountOut {opportunityThorchain.amountOutEstimated/1e8}')
                    ...
                    if opportunityThorchain.amountInInDollars > 15:
                        ...
                        if (
                            ...
                            # listSuccessOpportunitiesOnBlock.append(opportunityThorchain)
                            queueOpportunitiesThorchain.put(opportunityThorchain)
                            print("y'a une opp ONBLOCK THORCHAIN fréro")
        ...
        logging.info(f'SET eventScoutOnblockThorchain')
    ...
    try:
        ...
        for pairCexDex in pairsCexDex:
            ...
            if pairCexDex.pairAssetDex.assetIn.balance > 0:
                    # logging.info(f' amountInMax {amountInMax} assetIn {pairCexDex.pairAssetDex.assetIn.symbol} assetOut {pairCexDex.pairAssetDex.assetOut.symbol}')
                    ...
                    if opportunityMaya.amountInInDollars > 15.0:
                        ...
                        # logging.info(f'CEXDEXMAYA - gainTotalEstimated {round(opportunityCexDex.gainTotalEstimated,2)} $ - THRESHOLD_GAIN_NET_MAYA {THRESHOLD_GAIN_NET_MAYA} - THRESHOLD_GAIN1000_NET_MAYA {THRESHOLD_GAIN1000_NET_MAYA}')    
                        
                        if conditionOpp:
                            print("y'a une opp CEXDEX MAYA fréro")
            ...
            if pairMaya.assetIn.assetType == pairMaya.assetOut.assetType:
                if pairMaya.assetIn.balance >0:
                    ...
                    if opportunityMaya.amountInInDollars > 15:
                        ...
                        # logging.info(f'Expected Amount Out: {expected_amount_out}')
                        # logging.info(f'Outbound Fee: {outbound_fee}')

                        # logging.info(f'Slip fees : {liquidity}')
                        ...
                        if conditionOpp:
                            print("y'a une opp ONBLOCK MAYA fréro")
            ...
            if pairCexDex.pairAssetDex.assetIn.balance > 0 and pairCexDex.pairAssetCex.assetIn.balance > 0:
                for i in range(100): 
                    ...
                    if opportunityThorchain.pairAsset.assetIn.balance > amountInInAsset:
                        ...
                        if conditionOpp:
                            print("SCOUT - y'a une opp CEXDEX THORCHAIN fréro")
                            logging.info(f"")
                            logging.info(f'SCOUT CEXDEX - opp thorchain : assetIn : {opportunityCexDex.opportunityThorchain.pairAsset.assetIn.symbol} assetOut : {opportunityCexDex.opportunityThorchain.pairAsset.assetOut.symbol} amountIn {opportunityCexDex.opportunityThorchain.amountIn/1e8} amountOut {opportunityCexDex.opportunityThorchain.amountOutEstimated/1e8}')
...
def processCreateOpportunityMayaVSThor(balancesShared : Balances, queueOpportunitiesMaya:Queue, eventScoutDexDex:Event):
    try:
        ...
        for asset in balancesMaya.listAssets:
            if asset.poolName == 'THOR.RUNE':
                asset.balance = 1000*1e8
        ...
        for pairDexDex in pairsDexDex:
            ...
            # logging.info(f'MAYA : {pairDexDex.pairAssetDex1.assetIn.symbol }  {pairDexDex.pairAssetDex1.assetOut.symbol }')
            # logging.info(f'THOR : {pairDexDex.pairAssetDex2.assetIn.symbol }  {pairDexDex.pairAssetDex2.assetOut.symbol }')
            ...
            # logging.info(f'amountInMax {amountInMax} MAYA : {pairDexDex.pairAssetDex1.assetIn.symbol }  {pairDexDex.pairAssetDex1.assetOut.symbol }')

            if pairDexDex.pairAssetDex1.assetIn.balance > 10:
                ...
                opportunityThorchain = createThorchainOpportunityForMaya(pairThorchain=pairDexDex.pairAssetDex2, amountIn=amountInThorchain, balancesThorchain=balancesThorchain, typeOpp="THORMAYA")
