...
def getBlockPoolModifications(data):
    ...
    if 'events' in data['result']:
        ...
        for swap_coin, swap_emit in zip(swap_coin_events, swap_emit_events):
            ...
            logging.info(f'Swap Maya : {swapModification}')
...
def on_close(ws, close_status_code, close_msg):
    logging.info("### WebSocket Maya Closed ###")
...
def reconnect(ws):
    if not ws.sock or not ws.sock.connected:
        ...
        logging.info("WebSocket Maya is already open")
...
def on_open(ws):
    def run(*args):
        ...
        logging.info("Subscribed to Maya NewBlock events")
    threading.Thread(target=run).start()




def startWebsocketMaya(BalancesShared: Balances,orderbookDataShared: Dict,queueOpportunitiesmayachain: Queue, queueOpportunitiesToExecute: Queue,  balanceDictShared:Dict, poolDictShared:Dict):
    ...
    def process_message(message):
        try:
            ...
            # logging.info(f'Maya pool : {currentPool}')
            ...
            if 'result' in data:
                ...
                updatedPools = getMayaPools()
            
            # logging.info(f"Final updatedPools Maya : {type(updatedPools)}, Final updatedPools Maya : {updatedPools}" )
                
            process = multiprocessing.Process(
            target=processGetMayaBlock,
            args=(
                tendermintBlock,
                updatedPools,
                BalancesShared,
                orderbookDataShared,
                queueOpportunitiesmayachain,
    ...
    try:
        ...
        print("WebSocket Maya process terminated.")
    finally:
